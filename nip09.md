...
A special event with kind `84`, meaning "participant removal request" is defined as having a list of one or more `e` or `a` tags, each referencing an event from which the author wishes to be removed as a participant. Participant removal requests SHOULD include a `k` tag for the kind of each referenced event.

The event's `content` field MAY contain a text note describing the reason for the removal.

Unlike a kind `5` deletion request, a kind `84` event does not request deletion of the referenced event itself. It signals that the author of the kind `84` event wishes to be removed from the referenced event's participant list (i.e., from its `p` tags).
...
A client MUST validate that the `pubkey` of the kind `84` event appears in a `p` tag of each referenced event before honoring the removal. Clients SHOULD treat the participant as removed and stop displaying them in participant lists for the referenced events. Clients SHOULD also hide the referenced events from the removal author's own views (e.g., calendar, feed).

A kind `5` deletion request takes precedence over a kind `84` participant removal. If an event has been deleted via kind `5`, the kind `84` event has no additional effect.

### Relay Usage

Relays SHOULD store and serve kind `84` events indefinitely, as clients may need them to correctly render participant lists. Relays are not required to validate the participant relationship, as they may not have knowledge of all referenced events.

See [NIP-09x](nip09x.md) for the full specification.
